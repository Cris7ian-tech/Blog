
import { useNavigate } from "react-router-dom";
import ProjectCard from "../components/ProjectCard.tsx";

import reactLogo from "../assets/react.svg";
import pokedexImg from "../assets/pokedexImg.gif";
import hooksImg from "../assets/hooksImg.jpg";
import uiExpImg from "../assets/uiExImg.jpg";

const MainLayout = () => {
  const navigate = useNavigate();

  const projects = [
    {
      title: "Pokedex Explorer",
      description: "Explorá todos los Pokémon: búsqueda, paginación, filtros y detalles.",
      image: pokedexImg,
      link: "/pokemons",
    },
    {
      title: "React Hooks Series",
      description: "Artículos profundizando en los Hooks de React.",
      image: hooksImg,
      link: "/blog/hooks",
    },
    {
      title: "UI Experiments",
      description: "Animaciones, microinteracciones y experimentos visuales.",
      image: uiExpImg,
      link: "/projects/ui-experiments",
    }
  ];

  return (
    <main className="min-h-screen p-4 sm:p-6 md:p-12 lg:p-20">
      <div className="max-w-7xl mx-auto">


        {/* PRESENTACIÓN */}
        <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
          <div className="md:col-span-2 rounded-3xl overflow-hidden bg-slate-700">
            <img
              src={reactLogo}
              alt="react logo"
              className="w-full h-full object-cover"
            />
          </div>

          <div className="md:col-span-3 rounded-3xl bg-[#eb7d69] px-4 py-6">
            <h2 className="text-slate-100 text-2xl font-bold py-2">
              Hola! soy Cristian ✌️
            </h2>

            <p className="text-lg text-slate-100 mb-4 leading-relaxed">
              Soy un <strong>desarrollador web en proceso</strong> y este es mi blog.
            </p>

            <p className="text-lg text-slate-100 leading-relaxed">
              Acá comparto mis proyectos, experimentos y aprendizajes.
            </p>
          </div>
        </div>

        {/* PROYECTOS */}
        <section className="mt-14">
          <h2 className="text-3xl font-bold text-slate-100 mb-6">
            Proyectos destacados
          </h2>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {projects.map((p) => (
              <ProjectCard
                key={p.title}
                title={p.title}
                description={p.description}
                image={p.image}
                onClick={() => navigate(p.link)}
                />
            ))}
          </div>
        </section>

      </div>
    </main>
  );
};

export default MainLayout;